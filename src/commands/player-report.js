...
module.exports = {
  ...
  async execute(interaction) {
    try {
      const reportChannelId = process.env.PLAYER_REPORT_CHANNEL; // Channel ID for player reports
      const logChannelId = process.env.PLAYER_REPORT_LOG_CHANNEL; // Channel ID for logging reports
      const guildId = interaction.guild.id; // Guild ID for fetching emojis
      const adminRoleId = process.env.Admin_ID; // Admin role ID
      const moderatorRoleId = process.env.MODERATOR_ID; // Moderator role ID
